...
const is$typed = _is$typed,
  validate = _validate
const id = 'sh.tangled.repo.archive'
...
export type QueryParams = {
  ...
  ref: string
  /** Archive format */
  format?: 'tar' | 'zip' | 'tar.gz' | 'tar.bz2' | 'tar.xz'
...
export class ArchiveError extends XRPCError {
  constructor(src: XRPCError) {
    super(src.status, src.error, src.message, src.headers, { cause: src })
  }
}
...
export function toKnownErr(e: any) {
  if (e instanceof XRPCError) {
    ...
    if (e.error === 'ArchiveError') return new ArchiveError(e)
  }
