...
const is$typed = _is$typed,
  validate = _validate
const id = 'sh.tangled.repo.branch'
...
export class BranchNotFoundError extends XRPCError {
  constructor(src: XRPCError) {
    super(src.status, src.error, src.message, src.headers, { cause: src })
  }
}
...
export function toKnownErr(e: any) {
  if (e instanceof XRPCError) {
    ...
    if (e.error === 'BranchNotFound') return new BranchNotFoundError(e)
...
export interface Signature {
  $type?: 'sh.tangled.repo.branch#signature'
  /** Author name */
  name: string
  /** Author email */
  email: string
